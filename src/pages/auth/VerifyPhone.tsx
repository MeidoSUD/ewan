...
const VerifyPhone: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const qId = searchParams.get('user_id') || searchParams.get('userId') || searchParams.get('id') || '';
    // Then try pending_verification in localStorage (set by signup/register)
    let pending = null;
    try {
      pending = typeof window !== 'undefined' ? JSON.parse(localStorage.getItem('pending_verification') || 'null') : null;
    } catch (e) {
      pending = null;
    }

    const pendingPhone = pending?.phone_number || pending?.phone || '';
    const pendingId = pending?.user_id || pending?.user?.id || pending?.id || '';

    setPhone(p || pendingPhone || '');
    setUserId(qId || pendingId || '');
  }, [searchParams]);
  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      else if (phone) payload.phone_number = phone.replace(/^\+/, '');
  ...
  return (
    <div className="mx-auto max-w-md py-10 px-4">
      ...
      <p className="mb-4">A verification code was sent to {phone || 'your phone'}</p>
      {userId && <p className="mb-4 text-sm">Verifying account id: <strong>{userId}</strong></p>}
